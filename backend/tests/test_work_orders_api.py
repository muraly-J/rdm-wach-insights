...
def test_sendback_work_order_from_forced_status(client):
    resp = client.post("/api/work-orders/1/sendback")
...
def test_push_to_engineers(client):
    resp = client.post("/api/work-orders/1/push-to-engineers")
    assert resp.status_code == 410
...
def test_start_work_order(client, tmp_path, monkeypatch):
    ...
    db.update_work_order(wo_id, status="open", assigned_to="any")
    import core.agentdb as m
...
def test_list_work_orders_filter_assigned_to(client, tmp_path, monkeypatch):
    ...
    db.update_work_order(wo1, status="open", assigned_to="any")
    db.update_work_order(wo2, status="open", assigned_to="999")
    import core.agentdb as m
...
def test_sendback_work_order(client):
    resp = client.post("/api/work-orders/1/sendback")
    assert resp.status_code == 410
