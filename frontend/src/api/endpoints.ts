...
export const ENDPOINTS = {
  ...
  DASHBOARD_RANKING: (level: number, range: string) => `/dashboard/ranking?level=${level}&range=${range}`,
  DASHBOARD_TREND: (level: number, range: string) => `/dashboard/trend?level=${level}&range=${range}`,
  DASHBOARD_SUMMARY: (level: number, range: string, ahuId?: string) => `/dashboard/summary?level=${level}&range=${range}${ahuId ? `&ahu_id=${ahuId}` : ''}`,
  ...
  RAW_SCORE_RELATIONSHIP: (deviceId: string, range: string) => `/device/${deviceId}/raw-score-relationship?range=${range}`,
