...
export const METRIC_META: Record<string, { unit: string; label: string; description: string }> = {
  ...
  apparent_power_total: { unit: 'kVA',   label: 'Apparent Power',       description: 'Total apparent power' },
  reactive_power_total: { unit: 'kVAR',  label: 'Reactive Power',       description: 'Total reactive power' },
...
export const SCORE_METRIC_GROUPS: ScoreMetricGroup[] = [
  {
    ...
    availableMetrics: [
      'energy_import', 'power_total', 'power_l1', 'power_l2', 'power_l3', 'power_demand',
    ].map(toOption),
  },
  {
    ...
    availableMetrics: [
      ...
      'reactive_power_total', 'apparent_power_total', 'power_total',
    ].map(toOption),
  },
  {
    ...
    availableMetrics: [
      'current_unbalance', 'current_l1', 'current_l2', 'current_l3', 'current_avg',
      'volts_unbalance', 'volts_l1_n', 'volts_l2_n', 'volts_l3_n',
    ].map(toOption),
  },
  {
    scoreKey: 'thd_drift',
  ...
  },
  {
    ...
    availableMetrics: [
      ...
      'current_avg', 'current_l1', 'current_l2', 'current_l3', 'apparent_power_total',
    ].map(toOption),
  },
];
...
export const PREDICTION_CORE_METRICS = new Set([
  'energy_import', 'power_total', 'power_factor_avg', 'current_unbalance', 'composite_thd',
]);

// Cycling colors for mini-charts (avoids clash with main score colors)
export const MINI_CHART_COLORS = [
